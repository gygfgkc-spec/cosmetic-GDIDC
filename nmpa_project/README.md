...
## 1. 简介
这是一个专门用于抓取 **广东省特殊化妆品备案信息** 的自动化工具。它会自动打开浏览器，筛选“广东省”，并把所有状态为“当前批件”的产品信息（包括配方和使用目的）保存到 CSV 表格中（Excel 可打开）。
...
3. **抓取数据**：
   - 程序会自动一页页翻页。
   - 它会把数据实时保存在当前文件夹下的 `guangdong_cosmetics_v2.csv` 文件中。
...
- **Q: 运行报错说“无法提取产品名称”？**
  - A: 程序会自动在 `debug_screenshots` 文件夹下保存出错时的屏幕截图。请查看这些图片，如果图片里是一片白或者不是详情页，说明网络卡顿，请重试。如果图片正常但依然报错，请把截图发给开发人员。
